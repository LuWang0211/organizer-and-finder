...
async function main() {
  ...
  const roomNames = ['kitchen', 'living_room', 'bedroom_level_1', 'restroom_level_1', 'bedroom_level_2', 'restroom_level_2', 'closet'];
  const rooms = [];
  ...
  await prisma.item.createMany({
    data: [
      { name: 'LivingRoom Item 1', locationid: livingRoomLocations[0].id, quantity: 2 },
      { name: 'LivingRoom Item 2', locationid: livingRoomLocations[0].id, quantity: 4 },
      { name: 'LivingRoom Item 3', locationid: livingRoomLocations[0].id, quantity: 6 },
    ],
  });
