...
const RoomDescribers = {
  bedroom:
    "The bedroom contains common household items such as a bed, nightstand, wardrobe, and bookshelf, all in a blueprint style. The bed is placed centrally with a cozy, inviting look. There are small plants on the nightstand and a shelf, and a few toys scattered around, adding a playful touch.",
  "living room":
    "The living room includes typical elements such as a sofa, coffee table, TV stand, and bookshelf, all in the same blueprint style. The sofa is placed against one wall with a coffee table in front. There are plants on the coffee table and in the corners of the room, along with toys scattered playfully.",
  kichen:
    "The kitchen features a stove, refrigerator, sink, and dining table, all in a blueprint style. The stove is placed against one wall with a refrigerator next to it. The sink is located near the window, and the dining table is in the center of the room. There are small plants on the windowsill and a few toys on the floor, adding a playful touch.",
  restroom:
    "The bathroom includes a bathtub, toilet, sink, and mirror, all in a blueprint style. The bathtub is placed against one wall with the toilet next to it. The sink is located near the mirror, and there are small plants on the windowsill. The room has a clean, modern look with a touch of greenery.",
};
...
function generatePrompt() {
  ...
  let composedPrompt =
    "A top view of a loft layout blueprint inspired by the 2Advanced style. ";

  const allRoomNames =
    rooms.rooms
      .filter((room: any) => room.type === "room")
      .map((room: any) => `a ${room.name}`) ?? [];
  ...
  for (const room of rooms.rooms) {
    ...
    composedPrompt +=
      "**" +
      _.startCase(room.name) +
      ` (top left corner at ${room.x},${room.y}; size ${room.width}x${room.height} pixels):** `;
  ...
  for (const index in rooms.rooms) {
    ...
    composedPrompt +=
      "The area contains no furniture or items, serving as a passage between the rooms. ";
    composedPrompt += "\n\n";
  }
  ...
  for (const index in rooms.doors) {
    ...
    composedPrompt +=
      "The door is placed between two rooms, providing access between them. ";
    composedPrompt += "\n\n";
  }

  composedPrompt +=
    "The overall design is detailed and warm, with a family-friendly vibe, blending the modern and cozy with playful elements. ";

  composedPrompt +=
    `The picture you generated must depict exactly ${allRoomNames.length} number of rooms. Each room's top left corner must be at the specified coordinates and have the specified size. The door of each room must be at the specified location and have the specified size. \n` +
