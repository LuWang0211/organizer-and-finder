...
export function getRectangleEdges(
  ...
  const right = new Phaser.Geom.Line(
    rect.x + rect.width,
    rect.y,
    rect.x + rect.width,
    rect.y + rect.height,
  );
  const bottom = new Phaser.Geom.Line(
    rect.x + rect.width,
    rect.y + rect.height,
    rect.x,
    rect.y + rect.height,
  );
  const left = new Phaser.Geom.Line(
    rect.x,
    rect.y + rect.height,
    rect.x,
    rect.y,
...
function mergeGroupedVerticalLines(groups: {
  [key: string]: Phaser.Geom.Line[];
}): Phaser.Geom.Line[] {
  const mergedLines = [];
  ...
  for (const x in groups) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (currentLine.y2 >= line.y1) {
        ...
        currentLine = new Phaser.Geom.Line(
          currentLine.x1,
          currentLine.y1,
          currentLine.x2,
          Math.max(currentLine.y2, line.y2),
        );
      } else {
...
function mergeGroupedHorizontalLines(groups: {
  [key: string]: Phaser.Geom.Line[];
}): Phaser.Geom.Line[] {
  const mergedLines = [];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (currentLine.x2 >= line.x1) {
        ...
        currentLine = new Phaser.Geom.Line(
          currentLine.x1,
          currentLine.y1,
          Math.max(currentLine.x2, line.x2),
          currentLine.y2,
        );
      } else {
        // Push the current merged line and start a new one
...
export function mergeVerticalLines(
  lines: Phaser.Geom.Line[],
): Phaser.Geom.Line[] {
  lines = lines.map(
    ({ x1, y1, y2 }) =>
      new Phaser.Geom.Line(x1, Math.min(y1, y2), x1, Math.max(y1, y2)),
  );
...
export function mergeHorizontalLines(
  lines: Phaser.Geom.Line[],
): Phaser.Geom.Line[] {
  lines = lines.map(
    ({ x1, x2, y1 }) =>
      new Phaser.Geom.Line(Math.min(x1, x2), y1, Math.max(x1, x2), y1),
  );
...
export function mergeLines(lines: Phaser.Geom.Line[]): Phaser.Geom.Line[] {
  const [verticalLines, horizonalLines] = partition(
    lines,
