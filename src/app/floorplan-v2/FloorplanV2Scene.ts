...
export class FloorplanV2Scene extends Phaser.Scene {
    ...
    this.input.on(
      ...
      (pointer: any, gameObject: any, dragX: number, dragY: number) => {
        if (!this.isDrawingMode && (gameObject as any).stagingRectangle) {
          const stagingRect = (gameObject as any)
            .stagingRectangle as StagingRectangle;
          ...
          if ((gameObject as any).edgeType) {
            ...
            stagingRect.onEdgeResize(
              (gameObject as any).edgeType,
              worldPoint.x,
              worldPoint.y,
            );
          } else {
      ...
      if (!this.isDrawingMode) {
        if (currentlyOver.length > 0) {
          ...
          if ((clickedObject as any).stagingRectangle) {
            const stagingRect = (clickedObject as any)
              .stagingRectangle as StagingRectangle;
            this.selectRectangle(stagingRect);
          } else if ((clickedObject as any).stagingPolygon) {
            const stagingPolygon = (clickedObject as any)
              .stagingPolygon as StagingPolygon;
  ...
  handlePointerDown(pointer: Phaser.Input.Pointer) {
    ...
    if (!this.firstClickPoint) {
      ...
      this.createRectangle(this.firstClickPoint, {
        x: worldPoint.x,
        y: worldPoint.y,
      });
      this.cancelCurrentDrawing();
  ...
  handlePointerMove(pointer: Phaser.Input.Pointer) {
    if (!this.isDrawingMode || !this.firstClickPoint || !this.previewRectangle)
      return;
  ...
  createRectangle(
    ...
    const stagingRectangle = new StagingRectangle(
      this,
      centerX,
      centerY,
      width,
  ...
  combineRectangles(): void {
    ...
    if (
      uniqueRectangles.length > 1 &&
      !this.areAllRectanglesConnected(uniqueRectangles)
    ) {
      alert(
        "Warning: Not all rectangles are connected (overlap or touch). Cannot combine disconnected groups.",
      );
      return;
    }
  ...
  private removeDuplicateRectangles(): StagingRectangle[] {
    ...
    for (const rect of this.stagingRectangles) {
      ...
      const isDuplicate = unique.some((uniqueRect) => {
        ...
        return (
          Math.abs(bounds.x - uniqueBounds.x) < 0.1 &&
  ...
  private areAllRectanglesConnected(rectangles: StagingRectangle[]): boolean {
    ...
    for (let i = 0; i < rectangles.length; i++) {
      ...
      for (let j = 0; j < rectangles.length; j++) {
        adjacency[i][j] =
          i === j ||
          this.rectanglesOverlapOrTouch(rectangles[i], rectangles[j]);
      }
    }
  ...
  private rectanglesOverlapOrTouch(
    rect1: StagingRectangle,
    rect2: StagingRectangle,
  ): boolean {
  ...
  private unionRectanglesIntoPolygon(rectangles: StagingRectangle[]): void {
    ...
    const vertices: PolygonVertex[] = region.map((point) => ({
      x: point[0],
      y: point[1],
    }));
    ...
    if (this.floorplanV2Image) {
      ...
      this.load.once("complete", () => {
        if (this.floorplanV2Image) {
          ...
          this.currentBackgroundInfo.type = filename
            ? filename.replace(/\.[^/.]+$/, "")
            : textureKey;
          this.currentBackgroundInfo.floorplanPicture =
            filename || "custom-uploaded-image";
