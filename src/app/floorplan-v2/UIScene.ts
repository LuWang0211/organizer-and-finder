...
class PolygonThumbnail {
  ...
  constructor(
    scene: UIScene,
    polygon: StagingPolygon,
    mainScene: FloorplanV2Scene,
    targetWidth: number,
    targetHeight: number,
  ) {
    this.scene = scene;
  ...
  private createThumbnail(): void {
    ...
    this.boundingRect = this.scene.add.rectangle(
      0,
      0,
      this.targetWidth,
      this.targetHeight,
  ...
  private setGraphicsInteractiveArea(
    ...
    this.thumbnailGraphics.setInteractive(
      polygon,
...
class ShapesList {
  ...
  private updateShapesList(polygons: StagingPolygon[]): void {
    ...
    polygons.forEach((polygon, index) => {
      const thumbnail = new PolygonThumbnail(
        this.uiScene,
        polygon,
        this.mainScene,
        thumbnailWidth,
