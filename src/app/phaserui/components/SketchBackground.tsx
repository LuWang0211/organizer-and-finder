...
function distributeRectangles(W: number, H: number, rectangles: Rectangle[]): Rectangle[] {
    ...
    function getRandomPosition(rectWidth: number, rectHeight: number): { x: number, y: number } {
    ...
    rectangles.forEach(rect => {
        ...
        while (!placed && attempts < 100) { // limit attempts to avoid infinite loop
            ...
            let overlap = false;
            for (let placedRect of placedRectangles) {
        ...
        if (!placed) {
            console.error(`Could not place rectangle with width ${rect.width} and height ${rect.height}`);
        }
    });
...
export class SketchBackground {
    ...
    private createBackgroundForDesktop() {
        const glyphs = [
            ...
            "BPICON1", "BPICON2", "BPICON3", "BPICON3", 
        ]
        ...
        const placedRectangles = distributeRectangles(viewportWidth, viewportHeight, rectangles);

        for (const { x, y, data} of placedRectangles) {
            const glyph = this.glyphs[data];
