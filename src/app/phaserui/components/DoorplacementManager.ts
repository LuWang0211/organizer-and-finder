import { FloorPlanScene } from "@/app/phaserui/scenes/FloorPlanScene";
...
export class DoorplacementManager {
    ...
    create() {
        ...
        this.scene.input.on('drag', (pointer: Phaser.Input.Pointer, gameObject: Phaser.GameObjects.Rectangle,  dragX: number, dragY: number) =>
        {
            if (!includes(this.scene.doors, gameObject)) {
                return;
            }
            ...
            const intersectLines = this._doorPlacementRailsGroup!.getChildren().filter((sprite) => {
                let bounds = (sprite as Phaser.GameObjects.Zone).getBounds();
                
                return Phaser.Geom.Intersects.RectangleToRectangle(gameObjectRect, bounds);
            });
            ...
            if (intersectLines.length == 1) {
                ...
                const isHorizontal = intersectLine.data.get('isHorizontal') as boolean;
    ...
    public onFloorPlanContourChanged(mergedLines: Phaser.Geom.Line[]) {
        if (DEBUG) {
            if (!this.doorRailStrokes) {
                this.doorRailStrokes = this.scene.add.graphics({ lineStyle: { width: 2, color: 0x00ff00 } });
            } else {
